...
def import_entities(entities=[]):
    ...
    revisions = json.loads(pathlib.Path("data_mpr/reversion.json").read_text())
    ...
    for username in rev_users:
        if username:
            user_cache[username], _ = User.objects.get_or_create(
                username=username)
            ...
            for result in data["results"]:
                ...
                if "title" in result:
                    for title in result["title"]:
                        title_cache[title["label"].split(
                            ">>")[-1].strip()].append(result_id)
                    del result["title"]

                newentity, created = entitymodel.objects.get_or_create(
                    pk=result_id)
                for attribute in result:
                ...
                if result["source"] is not None:
                    if "id" in result["source"]:
                        sources[str(result["source"]["id"])
                                ]["content_type"] = content_type
                        sources[str(result["source"]["id"])
                                ]["object_id"] = newentity.id

                textids = [str(text["id"]) for text in result["text"]]
                entity_texts = {key: text for key,
                                text in texts.items() if key in textids}
                    ...
                    for field in newentity._meta.fields:
                        if field.verbose_name == text_type or field.name == text_type.lower():
                            ...
                            text_to_entity_mapping[key] = {
                                "entity_id": newentity.id, "field_name": field.name}
                    if not done:
                ...
                for new_history in newentity.history.all():
                    new_history.history_date = datetime.datetime(2017, 12, 31)
                    ent_revisions = list(filter(lambda x: is_entity(
                        x, str(result_id), entity), revisions.items()))
                    if ent_revisions:
                        revid, revision = ent_revisions[0]
                        timestamp = datetime.datetime.fromisoformat(
                            revision["timestamp"])
                        new_history.history_date = timestamp
                        if revision.get("user") is not None:
                            new_history.history_user = user_cache[revision["user"]]
                    new_history.save()
    ...
    for uri_id, uri in list((k, v) for k, v in uris.items() if v["entity"] in result_ids):
        ...
        try:
            ...
            print(f"Could not save uri {uri['uri']}: {e}")
