...
COPYFIELDS = ['review', 'start_date', 'start_start_date', 'start_end_date', 'end_date', 'end_start_date',
              'end_end_date', 'start_date_written', 'end_date_written', 'status', 'references', 'notes', 'published', 'source']
        ...
        try:
            ...
            if subj := relation["subj"]:
                ...
                if ct not in property_cache[property_identifier]["subj_class"]:
                    property_cache[property_identifier]["subj_class"].append(
                        ct)
            ...
            if subj and obj and prop:
                try:
                    ...
                    tt.save()
                    for tt_hist in tt.history.all():
                        tt_hist.history_date = datetime.datetime(2017, 12, 31)
                        rel_revisions = list(filter(lambda x: is_relation(
                            x, str(id), relation["type"]), revisions.items()))
                        if rel_revisions:
                            rid, revision = rel_revisions[0]
                            timestamp = datetime.datetime.fromisoformat(
                                revision["timestamp"])
                            tt_hist.history_date = timestamp
                            if revision.get("user") is not None:
                                tt_hist.history_user = user_cache[revision["user"]]
                        tt_hist.save()
